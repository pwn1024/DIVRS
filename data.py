...
class DIVRSFactorizationDataset(FactorizationDataset):

    def __init__(self, flags_obj, dm):

        super(DIVRSFactorizationDataset, self).__init__(flags_obj, dm)
    ...
    def make_sampler(self, flags_obj, dm):
        ...
        self.sampler = utils.DIVRSSampler(flags_obj, train_lil_record, train_dok_record, flags_obj.neg_sample_rate, dm.popularity, margin=flags_obj.margin, pool=flags_obj.pool)

        train_skew_coo_record = dm.skew_coo_record
        train_skew_lil_record = transformer.coo2lil(train_skew_coo_record)
        train_skew_dok_record = transformer.coo2dok(train_skew_coo_record)

        self.skew_sampler = utils.DIVRSSampler(flags_obj, train_skew_lil_record, train_skew_dok_record, flags_obj.neg_sample_rate, dm.popularity, margin=flags_obj.margin, pool=flags_obj.pool)
