...
class Recommender(object):
    ...
    def load_ckpt(self, epoch, model_path=None, test=False, model='normal'):
...
class DIVRSRecommender(MFRecommender):

    def __init__(self, flags_obj, workspace, dm):

        super(DIVRSRecommender, self).__init__(flags_obj, workspace, dm)

    def set_model(self):

        self.model = model.DIVRS(self.dm.n_user, self.dm.n_item, self.flags_obj.embedding_size, self.flags_obj.dis_loss, self.flags_obj.dis_pen, self.flags_obj.int_weight, self.flags_obj.pop_weight)

    def get_pair_dataloader(self):

        return data.FactorizationDataProcessor.get_DIVRS_dataloader(self.flags_obj, self.dm)
...
class DIVRS_GCNRecommender(DIVRSRecommender):

    def __init__(self, flags_obj, workspace, dm):

        super(DIVRS_GCNRecommender, self).__init__(flags_obj, workspace, dm)
        self.init_graph(flags_obj)
    ...
    def set_model(self):
        ...
        self.model = model.DIVRS_GCN(self.dm.n_user, self.dm.n_item, self.flags_obj.embedding_size, self.flags_obj.num_layers, self.flags_obj.dropout, self.flags_obj.dis_loss, self.flags_obj.dis_pen, self.flags_obj.int_weight, self.flags_obj.pop_weight)
