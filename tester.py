...
class Tester(object):
    ...
    def __init__(self, flags_obj, recommender, test_iou=True):
    ...
    def filter_history(self, items, train_pos):
        ...
        # processed_items = [items[i][np.isin(items[i], train_pos[i], invert=True)][:self.max_topk] for i in range(len(items))]
        # max_length = max(len(item) for item in processed_items)
        # processed_items = [np.pad(item, (0, max_length - len(item)), 'constant') for item in processed_items]
        # result = np.stack(processed_items, axis=0)
        try:
            result = np.stack([items[i][np.isin(items[i], train_pos[i], invert=True)][:self.max_topk] for i in range(len(items))],
                     axis=0)
        except:
            print("stop")
            result = None
