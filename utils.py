...
class ContextManager(object):
    ...
    def set_load_path(flags_obj):
        ...
        elif flags_obj.dataset == 'douban':
            flags_obj.load_path = const_util.douban
    ...
    def set_process_name(self):
        ...
        setproctitle.setproctitle(self.exp_name + '@XXX')
    ...
    def set_recommender(flags_obj, workspace, dm):
        ...
        elif flags_obj.model == 'DIVRS':
            return recommender.DIVRSRecommender(flags_obj, workspace, dm)
        ...
        elif flags_obj.model == 'DIVRS_GCN':
            return recommender.DIVRS_GCNRecommender(flags_obj, workspace, dm)
...
class DIVRSSampler(SAMPLER.Sampler):

    def __init__(self, flags_obj, lil_record, dok_record, neg_sample_rate, popularity, margin=10, pool=10):

        super(DIVRSSampler, self).__init__(flags_obj, lil_record, dok_record, neg_sample_rate)
    ...
    def generate_negative_samples(self, user, pos_item):
        ...
        mask_type = np.full(self.neg_sample_rate, False, dtype=np.bool_)
